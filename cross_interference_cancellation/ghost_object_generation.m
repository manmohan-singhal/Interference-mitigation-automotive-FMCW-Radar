...
intchirp  = txchirpParam; % aggressor radar's transmitted signal specifications

intchirp.start_time  = intchirp.duration/500;
intchirp.duration   = intchirp.duration*1.001;
% signal amplitude
...
% aggressor location
aggressor_range    = 5;    % aggressor radar range from the victim (assumed to be at same(near) location)
...
%[~,index] = sort(peak_bins_rdm(:,3),'descend');peak_bins_rdm(index(1:3),:)
